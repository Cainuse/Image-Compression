...
private:
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
	...
	long getSumHelper(vector<vector<long>> & channel, pair<int, int> ul, pair<int,int> lr);
...
public:
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
	...
	//debugging FUNCTIONS
	//these functions will take print only the first 5 rows of red cuz im to lazy to
	//implement channel selection and printing more rows would be too messy
	//
	//to use 1, dont use really big pictures ie not 1920X1080 and 2 in console
	//do ./pa3 > test.txt so you get a text file not in console
	void print_sum();

	void print_sumsq();

	void print_original(PNG & im);
};
