...
private:
   ...
   public:
      ...
      RGBAPixel avg;
      Node * left; // ptr to left subtree (contains parent's UL corner and new LR)
      Node * right; // ptr to right subtree (contains new UL and parent's LR)
...
private:
   ...
   */
   ...
   void copyTree(Node* & copy, Node* node);

   void addLeftTrees(Node* & copy, Node* node);

   void addRightTrees(Node* & copy, Node* node);

   void renderPic(PNG & png, Node* node);

   void pruneHelper(Node* curNode, double pct, int tol);

   bool pruneNode(Node* node, double pct, int tol);

   void findLeaves(Node* node, double & numLeaf, double & numWithinTol, int tol, Node* root);
   /**
   ...
   */
   ...
   void partitionLeft(stats & s, Node* & node, pair<int,int> ul, pair<int,int> lr);
   void partitionRight(stats & s, Node* & node, pair<int,int> ul, pair<int,int> lr);
